...
const SOCKET_URL = "wss://api.veloxiai.app/";
...
const MeshResult: React.FC<MeshResultProps> = ({id, embedded = false}) => {
    ...
    const version = mesh.aiVersion === "master" ? "V2" : mesh.aiVersion === "rodin" ? "V3" : "V1";

    return (
        <div className="w-full flex flex-col items-center relative pt-2 md:pt-0">
            <div
                className={`relative w-full aspect-video bg-primary-800 rounded overflow-hidden shadow-xl border border-accent-300 ${
                    isRefining ? "" : ""
                } max-h-[70vh]`}
            >
                <div className="absolute bottom-4 right-4 text-center font-semibold">
                    <p className="text-sm font-bold bg-accent-400 rounded text-white px-2">{version}</p>
                    {isRefining && (
